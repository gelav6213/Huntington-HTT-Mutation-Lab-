...
## Part 2. Gene and Normal Protein Function
...
## Part 4. Normal Reference Sequence
...
## Part 7. Methods: Creation of the Documented Mutant
...
## Part 9. WT versus Documented Mutant Protein
...
## Part 10. Molecular Interpretation: Gene → Mutation → Protein → Cellular Effect → Phenotype

The HTT gene contains the instructions for making the huntingtin protein. In Huntington disease, the HTT gene has an expanded CAG repeat. Since CAG codes for glutamine, the expansion adds more glutamine residues to the huntingtin protein. This produces an abnormally expanded protein that can affect normal cellular processes. These changes can lead to dysfunction of vulnerable neurons in the brain, which contributes to the motor, cognitive, and psychiatric symptoms seen in Huntington disease.

Overall pathway:
Gene → Mutation → Protein → Cellular effect → Phenotype:
HTT gene → CAG repeat expansion → expanded huntingtin protein → disruption of cellular processes → Huntington disease symptoms

## Part 11. Limitations
This study was mainly based on computer-based sequence analysis, so it could show the DNA and predicted protein changes but could not directly observe what happens inside cells. The analysis also could not directly measure protein aggregation, neuronal damage, or the patient's symptoms. Therefore, the cellular and disease effects were based on information from published studies rather than directly tested in this experiment.

## Part 12. Conclusion
This activity showed how a mutation can affect a gene and its protein. The CAG repeat expansion in the HTT gene adds glutamine residues to the huntingtin protein without changing the reading frame. Our sequence analysis showed that the mutant CDS and predicted protein were longer than the WT sequence. Overall, the activity helped demonstrate how a specific DNA mutation can lead to changes in a protein and contribute to disease.
...
National Center for Biotechnology Information. (n.d.). *HTT huntingtin [Homo sapiens (human)]*. NCBI Gene.

National Center for Biotechnology Information. (n.d.). *NM_002111.8 — HTT transcript*. NCBI Reference Sequence.
